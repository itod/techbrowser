...
- (NSString *)OAuthString {
	...
	if ([self port]) {
		...
		if (!(([lowercaseScheme isEqualToString:@"http"] && portInteger == 80) || 
			...
			port = [NSString stringWithFormat:@":%li", (long)portInteger];
		}
	}
...
+ (void)authorizeWithUsername:(NSString *)username password:(NSString *)password callback:(void (^)(BOOL))callback {
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		NSMutableDictionary *parameters = [NSMutableDictionary dictionaryWithObjectsAndKeys:username, @"x_auth_username", password, @"x_auth_password", @"client_auth", @"x_auth_mode", nil];
